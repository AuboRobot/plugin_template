...
#include "program/test_program_node_service.h"
#include "installation/test_installation_node_service.h"
...
private:
    ...
    void Start(BundleContext context)
    {
        ...
        // 安装节点服务类的注册方法如下
        props["Name"] = std::string(typeid(testInstallationNodeService).name());
        context.RegisterService<arcs::aubo_scope::InstallationNodeService>(
                    std::make_shared<testInstallationNodeService>(), props);

        // 程序节点服务类的注册方法如下
        props["Name"] = std::string(typeid(testProgramNodeService).name());
        context.RegisterService<arcs::aubo_scope::ProgramNodeService>(
                    std::make_shared<testProgramNodeService>(), props);
